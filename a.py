...
def bot(op):
            ...
            if op.param2 not in Bots:
              if op.param2 in Bots:
                ...
                try:
                  ...
                  ki.cancelGroupInvitation(op.param1, gMembMids)
                  ki.sendText(op.param1, "คุณไม่ได้รับสิทธิ์ในการเชิญ ฉันจึงขอยกเลิก 😛")
                  ki.kickoutFromGroup(op.param1,[op.param2])
    ...
    #-----------------=Selesai=------------------
              ...
              if msg.from_ in admin:
                ...
                ##cl.sendMessage(msg)
            elif msg.text in ["Buka qr","Oqr","ourl","Open","Buka"]:
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    ki.updateGroup(X)
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"สถานะ QR = เปิด")
                    else:
                ...
                if msg.toType == 2:
                    ...
                    #else:
                        #ki.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Tutup qr","Cqr","curl","Close","Tutup"]:
              if msg.from_ in admin:
                if msg.toType == 2:
                    X = ki.getGroup(msg.to)
                    X.preventJoinByTicket = True
                    ki.updateGroup(X)
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"สถานะ QR = ปิด")
                    else:
...
#----------------------------------------------
              ...
              if msg.from_ in admin:
                ...
                if wait["lang"] == "JP":
                    cl.sendText(msg.to,"กลุ่มทั้งหมดได้รับการปฏิเสธการเชิญแล้ว")
                else:
                ...
                #print wait2
                 ...
                 if msg.from_ in admin:
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "|| ตรวจสอบคนแอบอ่าน ||\n==================\n%s\n\n%s>=================<\n> 😀รักนะคนแอบอ่าน😀 <\n[%s]" % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
...
#-----------------------------------------------
    ...
    #-------------Fungsi Leave Group Start---------------#
            elif msg.text in ["ชิ้ว...!!!","Keluar","Kabur"]:
              if msg.from_ in admin:
                if msg.toType == 2:
                  cl.getGroup(msg.to)
                    ...
                    for g in gs.members:
                        ...
                        for target in targets:
                            try:
                                ...
                                cl.sendText(msg.to,"Done")
     #-------------       
            elif "CcccC" in msg.text:
               if msg.toType == 2:
                    print "hm"
                    _name = msg.text.replace("CcccC","")
                    gs = cl.getGroup(msg.to)
                    gs = ki.getGroup(msg.to)
                    #gs = ki3.getGroup(msg.to)
                    #gs = ki4.getGroup(msg.to)
                    #gs = ki5.getGroup(msg.to)
                    cl.sendText(msg.to,"Good Bye (*´･ω･*)")
                    targets = []
                    for g in gs.members:
                        if _name in g.displayName:
                            targets.append(g.mid)
                        ...
                        for target in targets:
                          if target not in Bots:
                             try:
                                 ...
                                 cl.sendText(msg.to,"Group cleanse (*´･ω･*)")
                ...
                if targets == []:
                  ...
                  for target in targets:
                    try:
                      ...
                      print e
        #-----------------------------------------
            elif "readyop" in msg.text:
              if msg.from_ in owner:
                if msg.toType == 2:
                    print "ok"
                    _name = msg.text.replace("readyop","")
                    gs = cl.getGroup(msg.to)
                    gs = ki.getGroup(msg.to)
                    #gs = ki2.getGroup(msg.to)
...
#-----------------------------------------------
                ...
                #ki2.sendText(msg.to,"Chomel pekok 􀜁??Har Har􏿿")
      ...
      #-------------Fungsi Balesan Respon Finish---------------------#
       ...
       #-------------Fungsi Speedbot Start---------------------#
              ...
              if msg.from_ in admin and owner:
                start = time.time()
                #cl.sendText(msg.to, "ระบบกำลังตรวจสอบ...")
