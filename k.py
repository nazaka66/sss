...
def bot(op):
    ...
    #-----------------=Selesai=------------------
              ...
              if msg.from_ in admin:
            ...
            elif msg.text in ["æ„›ã�®ãƒ—ãƒ¬ã‚¼ãƒ³ãƒˆ","Agift"]:
               if msg.from_ in admin:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
                                    'PRDTYPE': 'THEME',
               ...
               if msg.from_ in admin:
                ...
                ##cl.sendMessage(msg)
            elif msg.text in ["ourl","bot:ourl"]:
               if msg.from_ in admin:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
            ...
            elif msg.text in ["curl","bot:curl"]:
               if msg.from_ in admin:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.preventJoinByTicket = True
...
#-------------------- Protect Mode ------------
                ...
                if wait["Protectgr"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
#----------------------------------------------
            elif msg.text in ["Protect on","bot:on"]:
               if msg.from_ in admin:
                if wait["Protectjoin"] == True:
                    if wait["lang"] == "JP":
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["Protect off","bot:off"]:
               if msg.from_ in admin:
                if wait["Protectjoin"] == False:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"kick Joined Group Off")
               ...
               if msg.from_ in admin:
                if wait["Protectgr"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["bot:contact:on"]:
               if msg.from_ in admin:
                if wait["contact"] == True:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["bot:contact:off"]:
               if msg.from_ in admin:
            ...
            elif msg.text in ["bot:cancelall:on"]:
               if msg.from_ in admin:
                gid = cl.getGroupIdsInvited()
                for i in gid:
                    cl.rejectGroupInvitation(i)
                if wait["lang"] == "JP":
                    cl.sendText(msg.to,"เหม็ดฉ้าดแล่ว พี่น้องเฮ้อออออ")
                else:
...
#----------------------------- TAG ALL MEMBER -------------------------------
            if msg.text.lower() in ["@@@@"]:
                 ...
                 if jml > 500:
                    ...
                    cl.sendText(msg.to, "จำนวนประชากร :"+str(jml))
                    #cl.sendText(msg.to,"สาว่าอิถูกดังหล่าวนิ เรียกมาทั้งเพ")
